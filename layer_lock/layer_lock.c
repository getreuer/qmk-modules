...
/**
 * @file layer_lock.c
 * @brief Layer Lock community module implementation
 *
...
#pragma message \
    "getreuer/layer_lock: Layer Lock has evolved into core QMK feature Layer Lock! To use it, update your QMK set up and see https://docs.qmk.fm/features/layer_lock"

ASSERT_COMMUNITY_MODULES_MIN_API_VERSION(1, 0, 0);

#ifdef LAYER_LOCK_ENABLE
#error "getreuer/layer_lock: This module is an alternative to QMK core's built-in Layer Lock feature (`LAYER_LOCK_ENABLE = yes`). Please do not enable both at the same time."
#else
...
void housekeeping_task_layer_lock(void) {
  if (locked_layers &&
...
// Handles an event on an `MO` layer switch key.
static bool handle_mo(uint8_t layer, keyrecord_t* record) {
...
// Handles an event on a `TT` layer switch key.
static bool handle_tt(uint8_t layer, keyrecord_t* record) {
  if (record->event.pressed) {
    if (is_layer_locked(layer)) {
      layer_lock_invert(layer);  // On press, unlock the layer.
      return false;              // Skip default handling.
    }
  } else {
    if (is_layer_locked(layer) || !layer_state_is(layer)) {
      return false;  // Skip default handling.
    }
  }
  return true;
...
bool process_record_layer_lock(uint16_t keycode, keyrecord_t* record) {
  ...
  switch (keycode) {
    case QK_MOMENTARY ... QK_MOMENTARY_MAX:  // `MO(layer)` keys.
      return handle_mo(QK_MOMENTARY_GET_LAYER(keycode), record);

    case QK_LAYER_TAP_TOGGLE ... QK_LAYER_TAP_TOGGLE_MAX:  // `TT(layer)`.
      return handle_tt(QK_LAYER_TAP_TOGGLE_GET_LAYER(keycode), record);
...
bool is_layer_locked(uint8_t layer) {
  if (layer >= (sizeof(layer_state_t) * 8)) {
    return false;
  }
